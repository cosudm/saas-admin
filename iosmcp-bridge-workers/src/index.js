...
async function handleApi(request, env, url, path) {
  ...
  if (path === "/api/import/commit" && method === "POST") {
    ...
    await db.prepare("INSERT INTO sources (id,name,protocol,base_url,spec_text,spec_url,tool_count,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?)")
  ...
  const serverOut = async s => {
    const src = s.source_id ? await db.prepare("SELECT name,protocol,base_url,tool_count FROM sources WHERE id=?").bind(s.source_id).first() : null;
  ...
  if (m) {
    ...
    if (method === "PATCH") {
      ...
      if (body.base_url !== undefined && s.source_id) {
        await db.prepare("UPDATE sources SET base_url=? WHERE id=?").bind(body.base_url, s.source_id).run();
      }
      if (sets.length) {
    ...
    if (method === "DELETE") {
      ...
      if (s.source_id) await db.prepare("DELETE FROM sources WHERE id=?").bind(s.source_id).run();
  ...
  if (path === "/api/overview" && method === "GET") {
    ...
    return json({
      ...
      sources: (await db.prepare("SELECT COUNT(*) c FROM sources").first())?.c || 0,
  ...
  if (m && method === "GET") {
    const s = await findServer(m[1]);
    if (!s) return httpErr(404, "server not found");
    const src = s.source_id ? await db.prepare("SELECT * FROM sources WHERE id=?").bind(s.source_id).first() : {};
